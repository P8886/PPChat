...
const MessageInput = ({ onSubmit, onFocus, onImageUpload, allowImageUpload = true }) => {
  ...
  const submitOnEnter = (event) => {
    // 监听回车键
    if (event.keyCode === 13 && messageText.trim()) {
  ...
  const processImageFile = async (file) => {
    if (!allowImageUpload || !file) return
  ...
  const handlePaste = async (e) => {
    ...
    for (const item of items) {
      if (allowImageUpload && item.type.startsWith('image/')) {
        e.preventDefault()
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <input
        ...
        type="text"
        placeholder={allowImageUpload ? '发送消息... (Ctrl+V 粘贴图片)' : '发送消息...'}
        value={messageText}
